...
export const useAnalysisResults = () => {
  ...
  const analysisResults = useState<AIAnalysisResponse | null>(
    'analysisResults',
    () => null
  )
  ...
  const setAnalysisResults = (data: AIAnalysisResponse) => {
    ...
    if (import.meta.client) {
      try {
        ...
        console.warn(
          'Failed to store analysis results in session storage:',
          error
        )
      }
    }
  ...
  const clearAnalysisResults = () => {
    ...
    if (import.meta.client) {
      try {
        ...
        console.warn(
          'Failed to clear analysis results from session storage:',
          error
        )
      }
    }
