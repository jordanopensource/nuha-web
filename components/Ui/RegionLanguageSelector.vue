  ...
  <div>
    ...
    >
      <template v-if="buttonContent !== 'text'" #icon>
        <Icon
          v-if="showFlagInButton && currentRegionFlag"
          :name="currentRegionFlag"
          size="20"
        />
    ...
    >
      <div class="space-y-6">
        ...
        <div v-if="mode === 'both' || mode === 'language'">
          ...
          <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 sm:gap-2">
            <UiButton
              ...
              class="border"
              :class="{
                '!border-colors-primary !bg-colors-primary-light !bg-opacity-80':
                  locale === l.code,
                'border-colors-neutral-foreground border-opacity-20':
                  locale !== l.code,
              }"
              @click="updateLang(l.code)"
            >
              <div :class="l.dir === 'rtl' ? 'text-right' : 'text-left'">
                <div class="font-IBMPlexSansArabic font-medium">
                  {{ l.nativeName }}
                </div>
                <small>{{ l.name }}</small>
              </div>
        ...
        <div v-if="mode === 'both' || mode === 'region'">
          ...
          <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 sm:gap-2">
            <template v-for="r in supportedRegions" :key="r.countryCode">
              <UiButton
                v-if="r.dialectName && r.dialectName[locale]"
                variant="ghost"
                size="lg"
                class="region-btn !grid border border-colors-neutral-foreground border-opacity-20"
                :class="{
                  '!border-colors-primary !bg-colors-primary-light !bg-opacity-80':
                    region?.countryCode === r.countryCode ||
                    detectedRegion?.countryCode === r.countryCode,
                  '!border-opacity-10 !bg-colors-primary-light !bg-opacity-10 opacity-60':
                    !r.isAvailable,
                }"
        ...
        <div
          v-if="
            ...
            (mode === 'region' || mode === 'both')
          "
          class="rounded-lg bg-colors-primary-light bg-opacity-10 p-4"
        >
          <div class="mb-2 flex items-center gap-2">
            <Icon name="mdi:map-marker" size="16" />
            <span class="text-sm font-medium">{{
              $t('settings.regionLanguage.detectedLocation')
            }}</span>
          </div>
          <small class="ms-6">
            {{ detectedRegion.city ? `${detectedRegion.city}, ` : ''
            }}{{ detectedRegion?.countryCode }}
          </small>
        </div>
      </div>
...
<script setup lang="ts">
  ...
  const currentSelectionText = computed(() => {
    if (props.mode === 'language') {
      ...
      return props.showFlagInButton &&
        currentRegionFlag.value &&
        props.buttonContent === 'icon'
        ? ''
        : regionName.value ||
            detectedRegion?.value?.countryCode ||
            $t('settings.regionLanguage.selectDialect')
    } else {
      const langText = languageName.value
      const regText =
        props.showFlagInButton && currentRegionFlag.value
          ? ''
          : regionName.value
  ...
  const updateRegion = async (reg: string) => {
    try {
      if (reg && reg !== region.value?.countryCode?.toLowerCase()) {
        const newRegion = supportedRegions.value.find(
          (r) => r.countryCode === reg
        )
        if (newRegion) {
          setRegion(newRegion)
