<template>
  <div class="page-container">
    <UiPageHeading :title="$t('login.title')" use-h1 class="mx-auto !flex" />

    <div
      class="mx-auto flex w-full max-w-2xl flex-col gap-4 rounded-md border border-colors-primary-light bg-white p-4 lg:px-8"
    >
      <!-- Messages -->
      <UiMessage
...
<script setup lang="ts">
  ...
  const termsLink = infoLinks.find((link) =>
    link.path().includes('/terms')
  ) || { path: () => localePath('/terms') }
  const privacyLink = infoLinks.find((link) =>
    link.path().includes('/privacy')
  ...
  const handleEmailLogin = async () => {
    ...
    try {
      ...
      if (response.success) {
        messages.success.text = t('login.messages.emailSent', {
          email: email.value,
        })
