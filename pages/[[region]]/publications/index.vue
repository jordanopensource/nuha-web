<template>
  <div class="page-container">
    ...
    <div v-if="error" class="py-12">
      <UiMessage :message="$t('publications.fetchError')" type="error">
        <template #actions>
    ...
    <div class="space-y-8">
        ...
        <div v-if="categoriesData && !categoriesError" class="mb-4">
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <UiButton
              ...
              size="sm"
              :variant="
                selectedCategoryId === category.id ? 'primary' : 'ghost'
              "
      ...
      <!-- No Publications -->
      <div
        v-if="data && publications.length === 0"
          ...
          {{
            currentPage > 1
              ? $t(
                  ...
                  'No more publications available.'
                )
              : selectedCategoryId !== null
                ? $t(
                    'publications.noPublications',
...
<script lang="ts" setup>
  ...
  const featuredPublications = computed(
    () => publications.value.filter((pub) => pub.featured).slice(0, 2) // Show max 2 featured
  )
  ...
  const pagination = computed(() => {
    return (
      (data.value?.meta?.pagination as MetaResponsePaginationByPage) || null
    )
  })
